...
const BlogDetail = () => {
  ...
  return (
    <div className="flex flex-col items-center pb-40 w-10/12 mx-auto font-[Urbanist]">
      <div className="bg-[#f6fbe9] w-10/12 h-[480px] rounded-xl mt-20 border-[#E5F5BD] border flex flex-col items-center relative text-[#333333]">
    ...
    {blog.category}
      ...
      <Image 
        ...
        width={800} 
        height={450} 
        className="rounded mt-5 w-10/12"
      />
      <h1 className="font-bold text-[#262626] w-10/12 mt-10">{blog.title}</h1>

{blog.intro && (
        <p className="mt-8 text-gray-700 w-10/12">{blog.intro}</p>
      )}

      {/* Blog List Section */}
      {blog.list && blog.list.length > 0 && (
        <ol className="list-decimal pl-5 space-y-5 mt-5 w-10/12">
      ...
      {blog.summary && (
        <p className="mt-8 w-10/12 font-semibold">{blog.summary}</p>
      )}
    </div>
  );
};
