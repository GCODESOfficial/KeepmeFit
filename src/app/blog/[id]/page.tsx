...
const BlogDetail = () => {
	...
	return (
		<div className="flex flex-col items-center pb-40 w-10/12 mx-auto font-[Urbanist]">
			<div className="bg-[#f6fbe9] w-10/12 hidden h-[480px] rounded-xl mt-20 border-[#E5F5BD] border lg:flex flex-col items-center relative text-[#333333]">
			...
			<div className=" lg:hidden pb-10 rounded-xl  mt-10 flex flex-col border-[#E5F5BD] border items-center relative text-[#333333]">
				{/* Logo Div Positioned on Top */}
				...
				<div className="text-center mx-auto flex flex-col -mt-20 z-20 gap-8">
					<h1 className="font-bold text-2xl text-[#262626]">Our Blogs</h1>
					<h1 className="text-xs w-10/12 mx-auto">
						Welcome to the Blog section of Nutritionist, your trusted source for
						insightful articles, tips, and expert advice on nutrition and
						wellness. Here, we strive to provide you with engaging and
						informative content that will inspire and empower you to make
			...
			/>
			<h1 className="font-bold text-[#262626] lg:w-10/12 w-11/12 mt-10 text-sm lg:text-base">
				{blog.title}
			</h1>

			{blog.intro && (
				<p className="mt-8 text-gray-700 text-sm lg:text-base lg:w-10/12">{blog.intro}</p>
			)}
				...
				<ol className="list-decimal pl-5 space-y-5 mt-5 lg:w-10/12">
					{blog.list.map((item, index) => (
						<li key={index} className="flex flex-col">
							...
							<p className="text-gray-700 text-sm lg:text-base mt-1">{item.description}</p>
						</li>
					))}
				</ol>
			)}
			...
			{blog.summary && (
				<p className="mt-8 lg:w-10/12 font-semibold text-sm lg:text-base">{blog.summary}</p>
			)}
		</div>
	);
};
